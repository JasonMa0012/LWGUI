...
public class ShowEditorIcons : EditorWindow
{
    [MenuItem("LWGUI/Editor Icons", priority = -1001)]
    public static void EditorIconsOpen()
    {
#if UNITY_2018
        var w = GetWindow<ShowEditorIcons>("Editor Icons");
#else
        var w = CreateWindow<ShowEditorIcons>("Editor Icons");
#endif
        w.ShowUtility();
    ...
    void SearchGUI()
    {
        ...
        {
            if (getIsWide()) GUILayout.Space(10);
    ...
    private bool getIsWide() { return Screen.width > 550; }

    private bool getDoSearch() { return !string.IsNullOrWhiteSpace(search) && search != ""; }
    ...
    GUIContent GetIcon( string icon_name )
    {
        ...
        return (valid == null || valid.image == null) ? null : valid;
    }
    ...
    private void OnGUI()
    {
        ...
        if (getIsWide()) GUILayout.Space(3);
        ...
        using (var scope = new GUILayout.ScrollViewScope(scroll))
        {
            ...
            if( getDoSearch() ) iconList = iconContentListAll.Where( x => x.tooltip.ToLower()
